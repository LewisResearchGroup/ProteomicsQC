...
class CORSHandler(webapp2.RequestHandler):
    def cors(self):
        ...
        headers["Access-Control-Allow-Methods"] = "OPTIONS, HEAD, GET, POST, DELETE"
        headers[
...
class UploadHandler(CORSHandler):
    ...
    def write_blob(self, data, info):
        key = (
            urllib.quote(info["type"].encode("utf-8"), "")
            + "/"
            + str(hash(data))
            + "/"
        ...
        if IMAGE_TYPES.match(info["type"]):
            try:
                ...
                img.resize(width=THUMB_MAX_WIDTH, height=THUMB_MAX_HEIGHT)
                thumbnail_data = img.execute_transforms()
                thumbnail_key = key + THUMB_SUFFIX
                memcache.set(thumbnail_key, thumbnail_data, time=EXPIRATION_TIME)
    ...
    def handle_upload(self):
        ...
        for name, fieldStorage in self.request.POST.items():
            ...
            if self.validate(result):
                key, thumbnail_key = self.write_blob(fieldStorage.value, result)
                if key is not None:
                    ...
                    if thumbnail_key is not None:
                        result["thumbnailUrl"] = (
                            self.request.host_url + "/" + thumbnail_key
                        )
                else:
...
app = webapp2.WSGIApplication(
    [("/", UploadHandler), ("/(.+)/([^/]+)/([^/]+)", FileHandler)], debug=DEBUG
)
